...
def getScenes(fname):
    if (fname.find("rain") > 0):
		# In these cases, simply selecting the first file/set. It is the same for all stages! 
        scenes = [line.strip() for line in open(dataPath + "train0.txt", 'r')]
    elif (fname.find("est") > 0): 
        scenes = [line.strip() for line in open(dataPath + "test0.txt", 'r')]
    else: 
...
def readResults(rtfexp_name, scene_name, conn, depth, stages, stack_index):
    ...
    # INPUT DONE
    outputRTF = np.loadtxt("%s/%s RTFStackedForMyLorentzian_%dx%d_%d_%d-stage%d.dlm"%(rtfexp_name, scene_name, conn, conn, depth, stages, stack_index), delimiter='\t')
    #OUTPUT DONE    
    return groundTruth, inputRTF, outputRTF
    
def writeStats(RTF_experiment, conn, depth, setType, S, stack_index): 
    nameStr = "%s/Evaluation_Results_RTFTrainedForMyLorentzian_%dx%d_%d_%s-stage%d.txt"%(RTF_experiment, conn, conn, depth, setType, stack_index)
    np.savetxt(nameStr, S, delimiter='\t', fmt='%.8f'); 

def evaluateScenes(rtfexp_name, sceneSet, conn, depth, stack_index): 
    ...
    for scene in sceneSet:
        ...
        GT, iRTF, oRTF = readResults(rtfexp_name, scene, conn, depth, stages, stack_index); 
        diffIm1 = oRTF - GT; 
...
def visualizeScene(rtfexp_name, sceneName, conn, depth, stages, stageSwitch = True, deltaSwitch = True):
    plt.figure(figsize=(12,3)); 
    GT = np.zeros((200, 200))
    iRTF = np.zeros((200, 200))
    oRTF = np.ndarray((200, 200, stages))
    for stack_index in range(stages): 
        GT, iRTF, oRTF[:, :, stack_index] = readResults(rtfexp_name, sceneName, conn, depth, stages, stack_index); 
        vmin, vmax = np.min(GT), np.max(GT); 
        plt.subplot(231); plt.imshow(iRTF, interpolation='nearest', vmin=vmin, vmax=vmax); plt.title("Input %s"%LorentzianLoss(iRTF, GT)); plt.colorbar(); 
    ...
    plt.subplot(221); plt.imshow(GT, interpolation='nearest', vmin=vmin, vmax=vmax); plt.title("Ground Truth %s"%sceneName); plt.colorbar(); 
...
def saveScene(sceneName, deltaSwitch = True):
    ...
    plt.subplot(131); plt.imshow(iRTF, interpolation='nearest', vmin=vmin, vmax=vmax); plt.axis('off'); plt.title("Input %s"%LorentzianLoss(f, gt)); plt.colorbar(); 
    plt.subplot(132); plt.imshow(oRTF, interpolation='nearest', vmin=vmin, vmax=vmax); plt.axis('off'); plt.title("Output %s"%LorentzianLoss(Dd, gt)); plt.colorbar(); 
...
#trainedType = " RTFTrainedForMyLorentzian_1x1_7" # note the leading space
    ...
    print("Enter RTF experiment name, connectivity, depth and stages (separate by return)")
    RTF_experiment, RTF_connectivity, RTF_depth, stages = getVal("1x1_10_lambda50_E2"), int(getVal("1")), int(getVal("10")), int(getVal("0")); 
    
    for i in range(stages):
        trainTotal_RTF_loss, trainTotal_formula_loss, Strain = evaluateScenes(RTF_experiment, trainScenes, RTF_connectivity, RTF_depth, i); 
        writeStats(RTF_experiment, RTF_connectivity, RTF_depth, "train", Strain, i)
        testTotal_RTF_loss, testTotal_formula_loss, Stest = evaluateScenes(RTF_experiment, testScenes, RTF_connectivity, RTF_depth, i)
        writeStats(RTF_experiment, RTF_connectivity, RTF_depth, "test", Stest, i)
    ...
    for scene in np.random.permutation(testScenes)[0:n]:
        visualizeScene(RTF_experiment, scene, RTF_connectivity, RTF_depth, stages, True, True);   
